document.addEventListener('DOMContentLoaded', function () {
    ...
    // narrow (owner-input-narrow) without clipping - but the full option list still needs each
    // country's name to be usable, so the currently-selected option's own displayed text is
    // swapped between the two forms rather than actually changing what's stored (option.value,
    // and so the posted phone_country_code, never changes). Per Thomas 2026-09-08.
    var select = document.querySelector('.owner-contact-details-form select[name="phone_country_code"]');
    if (!select) return;
